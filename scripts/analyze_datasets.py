...
Analyzuje deduplikovaný soubor datasets_dedup.jsonl
a vytváří agregace + plochý CSV výpis.

Výstupy:
- timeline.tsv                  (datasets podle roku)
- repos_datacite.tsv            (repozitáře/publisher/resourceTypeGeneral)
- repos_crossref.tsv            (Crossref member/publisher)
- orcid_coverage.json           (souhrn ORCID coverage)
- orcid_by_institution.tsv      (ORCID coverage podle CZ institucí)
...
def author_ids_from_datacite(dc_rec: Dict[str, Any]) -> Tuple[Set[str], Set[str]]:
    ...
    for p in people:
        ...
        for ni in p.get("nameIdentifiers") or []:
            ...
            if scheme == "orcid":
                orcid = normalize_orcid(ni.get("nameIdentifier"))
                break

        family = (p.get("familyName") or "").strip().lower()
        given = (p.get("givenName") or "").strip().lower()
...
def author_ids_from_crossref(cr_rec: Dict[str, Any]) -> Tuple[Set[str], Set[str]]:
...
def extract_year(dc_rec: Optional[Dict[str, Any]],
                 cr_rec: Optional[Dict[str, Any]]) -> Optional[int]:
...
def main():
    ...
    parser.add_argument(
        "--dedup",
        required=True,
        help="Path to datasets_dedup.jsonl"
    )
    parser.add_argument(
        "--out-dir",
        default="analysis",
    ...
    parser.add_argument(
        "--ror-dump",
        help="Optional path to ROR dump (for institution names in ORCID stats)"
    )
    ...
    # Agregace
    timeline_total = defaultdict(int)
    timeline_datacite = defaultdict(int)
    timeline_crossref = defaultdict(int)

    repos_datacite = defaultdict(int)    # (client_id, publisher, resourceTypeGeneral) -> count
    repos_crossref = defaultdict(int)    # (member, publisher) -> count
    ...
    # Funders
    funders_datacite = defaultdict(int)   # (identifier, type, name) -> count
    funders_crossref = defaultdict(int)   # (doi, name) -> count

    # Licence
    licenses_datacite = defaultdict(int)  # (uri, ident, rights, open_flag) -> count
    license_dataset_summary = {"open": 0, "nonopen": 0, "none": 0}
    ...
    # ROR names (pro ORCID coverage podle instituce)
        ...
        for line in f:
            ...
            if dc:
                ...
                for r in rights_list:
                    ...
                    if open_flag:
                        has_open = True
                    key = (uri or "", ident or "", rights or "", open_flag)
                    licenses_datacite[key] += 1
                    # pro plochý CSV:
                ...
                for fr in attrs.get("fundingReferences") or []:
                    ...
                    key = (fid, ftype, fname)
                    funders_datacite[key] += 1

                key_repo = (dc_client_id or "", dc_publisher or "", dc_rtype or "")
                repos_datacite[key_repo] += 1
            ...
            if cr:
                ...
                for fr in cr.get("funder") or []:
                    ...
                    key = (fdoi, fname)
                    funders_crossref[key] += 1

                key_repo = (str(cr_member or ""), cr_publisher or "")
                repos_crossref[key_repo] += 1

            # --- Autoři / ORCID coverage ---
            authors_all_dc, authors_orcid_dc = set(), set()
            authors_all_cr, authors_orcid_cr = set(), set()
            if dc:
            ...
            # podle institucí
            for rid in ror_ids:
                inst_authors_all[rid].update(authors_all)
                inst_authors_orcid[rid].update(authors_orcid)
    ...
    with open(timeline_path, "w", encoding="utf-8") as f:
        ...
        for year in sorted(timeline_total.keys()):
            f.write(
                f"{year}\t{timeline_total[year]}"
                f"\t{timeline_datacite.get(year, 0)}"
                f"\t{timeline_crossref.get(year, 0)}\n"
            )
    ...
    orcid_cov = {
        ...
        "datasets_with_at_least_one_orcid_pct":
            (datasets_with_orcid / datasets_total * 100.0) if datasets_total else 0.0,
        ...
        "persons_with_orcid_pct":
            ...
            if all_authors_global else 0.0,
    }
    with open(os.path.join(args.out_dir, "orcid_coverage.json"), "w", encoding="utf-8") as f:
    ...
    with open(orcid_inst_path, "w", encoding="utf-8") as f:
        f.write("ror_id\tname\tpersons_total\tpersons_with_orcid\tpersons_with_orcid_pct\n")
        for rid, authors_all_inst in sorted(
            inst_authors_all.items(),
    ...
    with open(funders_dc_path, "w", encoding="utf-8") as f:
        f.write("funderIdentifier\tfunderIdentifierType\tfunderName\tdataset_count\n")
    ...
    with open(os.path.join(args.out_dir, "license_dataset_summary.json"), "w", encoding="utf-8") as f:
